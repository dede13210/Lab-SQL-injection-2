...
const productSchema = new mongoose.Schema({
  ...
  price: Number,
  description: String,
  visible: Boolean // Ajout du champ visible dans le schéma
});
...
// Route for search with input validation and sanitization
app.get('/search', (req, res) => {
  ...
  // Ensure parsedQuery is an object to avoid further errors
  if (typeof parsedQuery !== 'object') {
    parsedQuery = {};
  }

  // Secure search query to only fetch visible products
  const finalQuery = {
    $and: [
      { visible: true },
      parsedQuery
  ...
  Product.find(finalQuery) // Perform the search with the secure query
    .then(products => {
